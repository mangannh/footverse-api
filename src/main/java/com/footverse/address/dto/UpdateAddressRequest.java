...
/**
 * Payload for {@code PUT /addresses/{id}} — a full update, so every recipient field is required.
 * Validation follows validation-spec §5; the exactly-one-default-per-user rule behind
 * {@code isDefault} is a business rule enforced by the service.
 *
 ...
 * @param isDefault      optional; when {@code true} the address becomes the caller's default and
